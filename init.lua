...
local properties = {
    "width", "height", "paused_bg_color", "active_bg_color", "fg_color",
    "seconds", "do_notify"
}
...
function awmodoro:begin()
    if not data[self].timer.started then
        ...
        if data[self].do_notify then
            naughty.notify({text = "Pomodoro Begin"})
            play_sound()
        end
    end
end
...
function awmodoro:pause()
    ...
    update(self)
    if data[self].do_notify then
        naughty.notify({text = "Pomodoro Paused"})
        play_sound()
    end
end
...
function awmodoro:resume()
    ...
    data[self].timer:start()
    if data[self].do_notify then
        naughty.notify({text = "Pomodoro Resumed"})
        play_sound()
    end
end
...
function awmodoro:reset()
    ...
    update(self)
    if data[self].do_notify then
        naughty.notify({text = "Pomodoro Reset"})
        play_sound()
    end
end
...
function awmodoro.new(args)
    ...
    local fg_color = args.fg_color or {
        type = "linear",
        from = {0, 0},
        to = {width, 0},
    ...
    local bar = wibox.widget.progressbar.new(
                    {forced_width = width, forced_height = height})
    ...
    data[_awmodoro] = {
        ...
        paused_bg_color = paused_bg_color,
        do_notify = do_notify
    }

    if args.finish_callback then
        data[_awmodoro].finish_callback = args.finish_callback
    end
    if args.begin_callback then
        data[_awmodoro].begin_callback = args.begin_callback
    end
    ...
    _awmodoro:buttons(awful.util.table.join(
            ...
            _awmodoro:toggle()
        end), awful.button({}, 2, function() _awmodoro:finish() end),
...
function awmodoro.mt:__call(...) return awmodoro.new(...) end
